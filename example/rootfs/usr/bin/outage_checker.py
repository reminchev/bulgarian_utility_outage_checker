...
class OutageChecker:
    ...
    def generate_config_snippet(self):
        ...
        try:
            ...
            sensor_config = f"""# Bulgarian Utility Outage Checker - Sensor
...
"""
            ...
            template_config = f"""# Bulgarian Utility Outage Checker - Binary Sensor
# Auto-generated by addon
- binary_sensor:
...
"""
            ...
            sensor_file = '/config/utility_outage_sensors.yaml'
            template_file = '/config/utility_outage_templates.yaml'
            ...
            logger.info("="*80)
            logger.info(f"📁 Sensor config: {sensor_file}")
            logger.info(f"📁 Template config: {template_file}")
            logger.info("="*80)
            logger.info("📋 ADD THESE LINES TO YOUR configuration.yaml:")
            logger.info("="*80)
            logger.info("")
            logger.info("sensor: !include utility_outage_sensors.yaml")
            logger.info("")
            logger.info("template: !include utility_outage_templates.yaml")
            logger.info("")
            logger.info("="*80)
            logger.info("💡 INSTRUCTIONS:")
            logger.info("="*80)
            logger.info("1. Open configuration.yaml in File Editor")
            logger.info("2. If you DON'T have 'sensor:' section, add:")
            logger.info("   sensor: !include utility_outage_sensors.yaml")
            logger.info("")
            logger.info("3. If you ALREADY have 'sensor:' section, change it to:")
            logger.info("   sensor: !include_dir_merge_list sensors/")
            logger.info("   Then move utility_outage_sensors.yaml to sensors/ folder")
            logger.info("")
            logger.info("4. If you DON'T have 'template:' section, add:")
            logger.info("   template: !include utility_outage_templates.yaml")
            logger.info("")
            logger.info("5. If you ALREADY have 'template:' section, change it to:")
            logger.info("   template: !include_dir_merge_list templates/")
            logger.info("   Then move utility_outage_templates.yaml to templates/ folder")
            logger.info("")
            logger.info("6. Save and Restart Home Assistant")
            logger.info("="*80)
            
        except Exception as e:
            logger.error(f"❌ Error generating config files: {e}")
